...
function getBundleDependencies (
  ...
  for (let module of modules) {
    ...
    let bundle = getParentBundle(id, bundles)
    if (!bundle) {
      const pluginSeparator = id.indexOf('!')
      // try looking upp the module that the plugin points to
      if (pluginSeparator > 0) {
        // plugin with a module top point at is a dependency itself
        const lastCharacter = id.length - 1
        const targetModule = pluginSeparator === lastCharacter
          ? id.substr(0, lastCharacter) : id.substr(pluginSeparator + 1)
        bundle = getParentBundle(targetModule, bundles)
        // add the target module instead of the plugin to the dependencies
        if (bundle) id = targetModule
      }
    }
    // do not modify dependencies of modules that are not bundled
    if (!bundle) {
    ...
    if (!visited.has(id)) {
      ...
      const { path, deps, dependents } = module
      module = { id, path }
      if (deps) {
